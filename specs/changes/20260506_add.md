...
- **一覧スコープ**: `UserRelationRepository.resolveForUser` は `{ kind: 'tenant', tenantId } | { kind: 'shops', userId }`。店舗割当ロールは **割当の存在確認のみ**（`LIMIT 1`）。顧客一覧の絞り込みは `TenantCustomerScope` / `ShopsCustomerScope` が **EXISTS＋JOIN（店舗側は `purchase_histories` ⟕ `shop_assignments` で `user_id` を束ねる）**により DB 内で完結させ、`shopIds[]` のメモリ展開と巨大 `IN` は使わない（詳細は §H）。
...
`CustomerRepository` / `ShopAccessRepository` が **`UserScopeResolution`**（`{ kind: 'tenant', tenantId }` または `{ kind: 'shops', userId }`）を返すために `admin_users` / `shop_assignments` を参照する。店舗割当ロールでは **`shop_assignments` の全件取得は行わず**、存在確認と **ユーザー ID のみ**を渡す。店舗一覧は `ShopRepository.listAssignedShopsForUser` が **`shops` INNER JOIN `shop_assignments`** で解決する。Gate 2 の単一リソース用 Resolver とは別経路だが、同じデータモデルに基づく（一覧スコープの詳細は §H）。
...
> **`AuthorizeOptions` は union 型**（`policy` または `relation` のどちらか一方は必須）。**どちらも欠ける場合は実行時 403（fallback-deny）**。以下は `worker/middleware/authorize.ts` と同一。
...
### H. CustomerScope の DB プッシュダウン・カーソルページネーション（追加 ADR）

#### ステータス

承認済み（本リポジトリに実装済み）。Gate 1/2 の全体方針（§2）とは独立した **データ層・一覧 API のスケーラビリティ**に関する追記である。

#### コンテキスト（解決した問題）

| # | 問題 | 概要 |
|---|------|------|
| 1 | インデックス不足 | `purchase_histories` / `shops` / `shop_assignments` の結合・フィルタ列に索引がなく、大規模データでフルスキャンになりうる。 |
| 2 | 全件ロード | `findAll()` 相当がスコープ内の全行を返し、メモリ・タイムアウトリスクがある。 |
| 3 | `shopIds` のアプリ持ち込み | `shop_assignments` を全件フェッチして `IN` / OR 分割していた。SQLite／D1 のバインド上限・クエリ肥大のリスク。 |
| 4 | `filterAccessibleIds` の逐次 await | `customerIds` チャンクを順次実行しており、並列化の余地があった。 |

#### 決定

**インデックス（マイグレーション）**

`purchase_histories(customer_id)`, `purchase_histories(shop_id)`, `shops(tenant_id)`, `shop_assignments(user_id, shop_id)` に複合・単列索引を追加する（例: `worker/rdb/migrations/0005_customer_scope_indexes.sql`）。

**`CustomerScope` インターフェース**

- `findAllCustomerRows()` を廃止し、`findCustomerRows(cursor: string \| null, limit: number)` とする。
- ソートは `customers.id` 昇順。カーソルは「直前ページ最終行の id」。`cursor == null` で先頭ページ、`WHERE id > cursor`、`LIMIT`。

**店舗スコープの SQL 完了**

- `UserScopeResolution` の shops ブランチは **`{ kind: 'shops', userId }`** とする。
- `UserRelationRepository` は店舗ロール時、`shop_assignments` を **`LIMIT 1` の存在確認のみ**（全 `shop_id` はフェッチしない）。
- `ShopsCustomerScope` の EXISTS は概ね次のイメージで、`user_id` はバインド 1 個に収める。

```sql
EXISTS (
  SELECT 1 FROM purchase_histories ph
  INNER JOIN shop_assignments sa
    ON sa.shop_id = ph.shop_id AND sa.user_id = ?
  WHERE ph.customer_id = customers.id
)
```

**店舗一覧（`ShopAccessRepository`）**

`AssignedShopsScope` は `shopIds[]` ではなく **`userId`** と `ShopRepository.listAssignedShopsForUser(userId)`（`shops` INNER JOIN `shop_assignments`）で割当店舗を取得する。

**ページネーションとリポジトリ**

- `CustomerRepository.findPage(cursor, limit)` が **`limit + 1` 件読み**して `nextCursor` を算出する（または同等の実装）。
- `GET /api/customers` はクエリ `cursor` / `limit`、応答 **`{ items, nextCursor }`**。既定 **`limit = 20`**、上限例 100（Zod）。
- `exportAll()` は `findPage` をチャンクループして結合する（応答ボディ全体が巨大な場合はストリーミング／非同期ジョブは別検討）。

**`filterAccessibleIds`**

同一ロジックのチャンク処理を **`Promise.all`** で並列実行する（店舗スコープ側の `shopIds` チャンクは JOIN により不要）。

**シード・E2E**

- 開発／テスト用に顧客を **合計 10,000 行**規模へ増やし、ページネーションを検証可能にする。
- **Cloudflare D1** はステートメントあたりのバインド数に厳しいため、`customers` は **約 20 行ずつ**、`purchase_histories` は **約 30 行ずつ**など列数に応じて INSERT を分割する。

**フロント（顧客一覧）**

- API と同期した **カーソルページネーション UI**（先頭へ・前へ・次へ、本ページ件数、`nextCursor` 有無の表示）。ページサイズは API 既定（20 件）に合わせる。

#### 結果（トレードオフ）

- EXISTS／JOIN 経路が索引でカバーされやすくなり、大量データでのフルスキャンリスクが下がる。
- 一覧が常にページ単位となり、一覧 API での OOM リスクが下がる。
- 店舗スコープのバインドが **`userId` 固定**となり、`IN` 分割が不要になる。
- インデックス維持コストと書き込みコストは増えるが、本機能は読みが主体として許容する。
- `exportCsv` 等の全件応答は別途、ストリーム／ジョブ化の検討余地あり。

#### 参照ファイル（実装の所在）

| 内容 | パス |
|------|------|
| `CustomerScope` IF | `shared/permission/scope/customer/scope.ts` |
| スコープ実装 | `worker/repository/customer-scope.ts` |
| 一覧／エクスポート | `worker/repository/customer.repository.ts` |
| ユーザー解決 | `worker/repository/user-relation.repository.ts` |
| 店舗一覧 JOIN | `worker/repository/shop.repository.ts`, `shop-access-scope.ts`, `shop-access.repository.ts` |
| HTTP / UseCase | `worker/routes/customers.ts`, `worker/usecase/customer.usecase.ts` |
| UI | `src/pages/Customers.tsx` |
| シード | `worker/test/helpers.ts`, `worker/routes/auth.ts`（`POST /api/auth/seed`） |
| 索引マイグレーション | `worker/rdb/migrations/0005_customer_scope_indexes.sql` |
