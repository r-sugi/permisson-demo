...
- **PBAC**に置くもの:
  ...
  - 「上限（数量制限）に達しているか」など **現在件数との比較**が要る判断は、PBAC の **宣言（上限値・可否フラグ）**までは Gate 1 で扱い、**実件数との突き合わせと拒否レスポンス**は **UseCase** 側に置く（DB 集計が要り、ミドルウェアに載せるとルート固有情報が増えるため）。理由の整理は [`02-request-authorization-pipeline.md`](./02-request-authorization-pipeline.md) を参照。
- **ReBAC**に置くもの:
